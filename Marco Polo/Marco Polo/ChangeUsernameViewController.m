...
-(void)changeUsername
{
    ...
    for(UITextField *input in tf)
    {
        ...
        if(i == tf.count -1)
        {
            ...
            if(self.username.text.length < 4|| self.username.text.length > 13)
            {
                [self showAlert:@"Username must be between 4 and 13 characters"];
            }
            else
            {
                ...
                [manager POST:@"http://combustionlaboratory.com/marco/php/updateUsername.php" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
                    ...
                    if ([status isEqualToString:@"one"])
                    {
                        ...
                        [self dismissViewControllerAnimated:YES completion:^{
                            [self.delegate userNameHasChanged];
                        }];
                    }
